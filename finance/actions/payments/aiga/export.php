...
foreach($center_office_ids as $center_office_id) {
    try {
        eQual::run('do', 'finance_payments_aiga_export-invoices', compact('center_office_id'));

        # todo - check if payments export needed
        // eQual::run('do', 'finance_payments_aiga_export-payments', compact('center_office_id'));
    }
    catch(Exception $e) {
