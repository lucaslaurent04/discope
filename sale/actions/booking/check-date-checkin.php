...
if(($booking['date_from'] > time()) && in_array($booking['status'], ['confirmed', 'validated']) && !$booking['is_cancelled'] && $center_office['id'] != 1  ) {
    $result[] = $booking['id'];
    $dispatch->dispatch('lodging.booking.date.checkin', 'sale\booking\Booking', $params['id'], 'warning', 'sale_booking_check-date-checkin', ['id' => $params['id']], [], null, $center_office['id']);
}
else {
    $dispatch->cancel('lodging.booking.date.checkin', 'sale\booking\Booking', $params['id']);
}
