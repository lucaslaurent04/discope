...
foreach($messages as $message) {
    if($message['object_class'] == 'sale\booking\Booking') {
...
if(count($map_bookings_ids)) {
    ...
    try {
        ...
        $result['logs'][] = 'Call `sale_booking_bulk-update-checks` on: '.implode(',', $bookings_ids);
        eQual::run('do', 'sale_booking_bulk-update-checks', ['ids' => $bookings_ids]);
    }
    catch(Exception $e) {
        // ignore errors
    }
