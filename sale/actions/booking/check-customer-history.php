...
if(isset($identity['flag_latepayer']) && $identity['flag_latepayer']) {
    // raise a notice
    $dispatch->dispatch('lodging.booking.latepayer_customer', 'sale\booking\Booking', $params['id'], 'notice', null, [], [], null, $booking['center_office_id']);
}
else {
    // symmetrical removal of the alert (if any)
    $dispatch->cancel('lodging.booking.latepayer_customer', 'sale\booking\Booking', $params['id']);
}

if(isset($identity['flag_damage']) && $identity['flag_damage']) {
    // raise a notice
    $dispatch->dispatch('lodging.booking.damage_customer', 'sale\booking\Booking', $params['id'], 'notice');
}
else {
    // symmetrical removal of the alert (if any)
    $dispatch->cancel('lodging.booking.damage_customer', 'sale\booking\Booking', $params['id']);
}

if(isset($identity['flag_nuisance']) && $identity['flag_nuisance']) {
    // raise a notice
    $dispatch->dispatch('lodging.booking.nuisance_customer', 'sale\booking\Booking', $params['id'], 'notice');
}
else {
    // symmetrical removal of the alert (if any)
    $dispatch->cancel('lodging.booking.nuisance_customer', 'sale\booking\Booking', $params['id']);
}
