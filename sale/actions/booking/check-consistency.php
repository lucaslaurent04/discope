...
$booking = Booking::id($params['id'])
    ->read([
            'id', 'name', 'status', 'is_cancelled', 'center_office_id',
...
if($booking && in_array($booking['status'], ['option', 'confirmed'])) {
    ...
    if($has_error) {
        $result[] = $params['id'];
        // raise a warning
        $dispatch->dispatch('lodging.booking.consistency', 'sale\booking\Booking', $params['id'], 'important', 'sale_booking_check-consistency', ['id' => $params['id']], [], null, $booking['center_office_id']);
    }
    else {
        // symmetrical removal of the alert (if any)
        $dispatch->cancel('lodging.booking.consistency', 'sale\booking\Booking', $params['id']);
    }
}
