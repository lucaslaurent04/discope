...
if($booking['mails_ids']) {
    ...
    if($last_email['status'] != 'sent') {
        $result[] = $booking['id'];
        $dispatch->dispatch('lodging.booking.email.send', 'sale\booking\Booking', $params['id'], 'warning', null, [], [], null, $booking['center_office_id']);
    }
    else{
        $dispatch->cancel('lodging.booking.email.send', 'sale\booking\Booking', $params['id']);
    }
}
