...
$tests = [
    ...
    '0901' => [
        ...
        'act' => function($data) {
            ...
            list($center, $booking_type_id, $customer_nature_id, $customer_identity_id, $rate_class_id) = $data;
            ...
            $pack_secondary_sojourn = Product::search(['sku', 'like', '%GA-SejScoSec-A%'])
                    ->read(['id','label'])
                    ->first(true);
        ...
        'assert'        => function($booking_lines) {
            $adult_product_lines = [];
            $children_product_lines = [];

            $adult_age_range_id = 1;
            $children_age_range_id = 2;
            foreach($booking_lines as $line) {
                if($line['product_id']['age_range_id'] === $adult_age_range_id) {
                    $adult_product_lines[] = $line;
                }
                elseif($line['product_id']['age_range_id'] === $children_age_range_id) {
                    $children_product_lines[] = $line;
                }
            }
            ...
            foreach($children_product_lines as $line) {
                if($line['free_qty'] !== 0) {
                    return false;
                }
            }
        ...
        'rollback'      => function() {
            Booking::search(['description', '=', 'Booking test 0901: adult freebies does not pass cap of nb_adult.'])->delete(true);
        }
    ],
    '0902' => [
