...
export class BookingFundingComponent implements OnInit, AfterContentInit {
  ...
  private async loadFunding() {
    const result:Array<any> = <Array<any>> await this.api.read("sale\\booking\\Funding", [this.funding_id], Object.getOwnPropertyNames(new Funding()));
    if(result && result.length) {
      const item:any = result[0];
  ...
  private async loadCustomer() {
    const result:Array<any> = <Array<any>> await this.api.read("sale\\customer\\Customer", [this.booking.customer_id], Object.getOwnPropertyNames(new Customer()));
    if(result && result.length) {
      const item:any = result[0];
  ...
  private async loadPayer() {
    const result:Array<any> = <Array<any>> await this.api.read("sale\\customer\\Customer", [this.booking.payer_organisation_id], Object.getOwnPropertyNames(new Customer()));
    if(result && result.length) {
      const item:any = result[0];
