...
export class BookingCompositionComponent implements OnInit, AfterContentInit, OnDestroy {
   ...
   */
    ...
    private async load(fields: any[]) {
        const result = <Array<any>> await this.api.read("sale\\booking\\Booking", [this.booking_id], fields);
    ...
    public onGenerate() {
        ...
        dialogRef.afterClosed().subscribe( async (result) => {
            if(result) {
                this.loading = true;
                const data:any = await this.api.fetch('?do=sale_booking_composition_generate&booking_id='+this.booking_id);
    ...
    public async onFileSelected(event:any) {
        ...
        if(file) {
            ...
            try {
                ...
                const response:any = await this.api.call('?do=sale_booking_composition_import', {
                    name: file.name,
                    type: file.type,
                    data: data,
